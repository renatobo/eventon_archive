...
User-facing docs are in `README.md`, per-version notes in `CHANGELOG.md`. This file is about changing the code.

**Grep for a claim before you change it.** This file states file counts, event counts, version numbers, test counts and byte sizes in more than one place, and a half-updated claim reads as authoritative. Sweep for every instance first, then re-sweep after editing. Both times that step was skipped, this file ended up contradicting itself.
...
**`grep` on this machine is ugrep, and `rg` is installed.** Two things to know before writing a scan: ugrep rejects an unescaped `[` in a pattern (`$_GET[` fails with "mismatched [ ]" — use `-F` or escape it), and zsh expands a bare `--include=*.php` before grep ever sees it, so quote it. `rg` avoids both and is what the `wordpress-skills` files assume.
...
`composer test`, or `vendor/bin/phpunit`. 61 unit tests, no database, no WordPress install, no Docker, ~15ms.
...
The suite is mutation-checked, not just green. Nine deliberate breakages were each confirmed to turn it red: `gmdate` → `date` on the year key, dropping the UTC zone in `build_item()`, removing the `limit` clamp, `is_default_view()` ignoring `limit`, an unknown `family` falling back to no filter, `nav` no longer being forced off for flat lists, and the counts signature gaining `limit` or losing `show` or `family`. **If you add a test, break the thing it covers and watch it fail before you trust it.**

Do the string surgery for a mutation inside Python, not through a bash heredoc or `argv`: `\t` passed that way arrives as a literal backslash-t and the replacement silently matches nothing. Assert the pattern was found. And parse PHPUnit output with `--colors=never`, or ANSI codes make `grep -E "^OK"` miss and a red run reads as no output at all.
...
- **WordPress core is readable locally at `~/development/wordpress_plugins/wordpress`** (7.0.2). Read it rather than guessing at core behaviour: it settled how `register_block_script_handle()` behaves with no `.asset.php`, that `wp_determine_option_autoload_value()` still maps the string `'no'`, and that the block-renderer route gates on `edit_posts`. Two assumptions that felt safe were wrong and this caught both. It is a release build, not a dev checkout, so there is no `tests/phpunit` and therefore no free integration harness.
...
- **`counts()` hashes only `family` and `show`.** Anything else in the signature would fragment the tally cache for no reason, and putting `limit` in it would let a five-row block report "5 bike nights". `build_counters()` reads `current_counts()` off the statics; the public `counts()` collects (or reads cache) and then does the same, so the strip and `[eventon_archive_count]` cannot disagree. Both go through `counts_signature()`, which `rebuild_now()` also uses to seed a tally from the collection it just ran — change the hash on one side only and the seed is silently orphaned. The tests pin this.
...
- **`rebuild_now()` builds before it discards, and that order is the point.** The obvious-looking `flush_cache(); render( array() );` is the bug it replaced: it deletes the option first, so a rebuild killed by `max_execution_time` on a 435-event site leaves nothing behind and the settings screen reports "never / 0", which reads as a broken plugin rather than an unfinished job. Both the settings button and cron go through it. Do not "simplify" this back — the failure only appears under a timeout, so no test will catch it.
- **One direct `$wpdb` query exists, in `pages_using_archive()`, and it is deliberate.** `WP_Query`'s `s` matches pages merely *mentioning* the shortcode in prose, and loading every published post to run `has_shortcode()` in PHP does not scale. Both `LIKE` values are literals through `esc_like()` and `prepare()`, `has_shortcode()`/`has_block()` re-check every hit, and it sits behind `manage_options` and a 5-minute transient. Nothing else here should acquire one without the same justification.
